...
# Get user input
if prompt := st.chat_input("Ask any xfinity related q's"):
    # Display user input
    with st.chat_message("user"):
        st.markdown(prompt)
        print(f"msg is={prompt}")
    ...
    with st.chat_message("assistant"):
        ...
        answer = response_data.get("messages", "No response from API")
        # st.markdown(f'**:red[{agent_dict[dialog_state]}]**')
        st.markdown(answer[-1]['content'])  # Display API response
        st.session_state.messages.append({"role": "assistant", "content": answer[-1]['content']})  # Save response
